...
trait ClientRoutes
extends All with Delete with Find with New with Update with ImageUpload with ImageDelete {
  ...
  val allClientRoutes = pathPrefix("client") {
    client_newRoute ~ client_findRoute ~ client_deleteRoute ~ client_updateRoute ~
      client_allRoute ~ client_imageuploadRoute ~ client_imagedeleteRoute
